...
class GTAgent:
    ...
    def __init__(self):
        ...
        system_prompt = (
            ...
            "다음은 당신이 생성한 프로필 GT에 대한 평가 기준입니다."
            "평가 기준을 고려하여 최대한 높은 점수를 받도록 프로필을 작성하세요."
            "1점 : 반환된 문서(채용공고)가 프로필 및 질문과 모두 무관"
            "2점 : 반환된 문서(채용공고)의 키워드가 프로필과 간접적이거나, 도메인은 비슷하지만 사용자가 원하는 도메인이 아님"
            "3점 : 반환된 문서(채용공고)가 프로필과 적합성이 있으나 질문과의 의도와 어긋남"
            "4점 : 핵심 도메인이 대부분 일치하고 반환된 문서(채용공고)가 질문 의도와 적합하나, 일부 요소가 반영이 안되어있음"
            "5점 : 사용자의 핵심 요구사항과 프로필 정보가 반환된 문서(채용공고)와 직접적으로 일치함"
        )
