...
RECOMMENDATION_GUIDANCE = (
    "\n\n**중요 지침:**\n"
    "1. 위에 제공된 10개의 채용공고는 벡터 검색으로 추출된 후보 공고들입니다.\n"
    "2. 각 공고를 꼼꼼히 검토하여 사용자의 프로필(전공·수강 과목·자격증·관심 직무)과 질문 의도에 실제로 적합한지 판단하세요.\n"
    "3. 적합한 공고가 있다면 가장 관련성 높은 1~3개를 선택하여 recommended_job_indices에 반환하세요. 예: [1, 3, 5]\n"
    "4. 선택한 각 공고에 대해 사용자 프로필을 바탕으로 추천 이유를 자세히 작성하세요.\n"
    "5. **중요**: 검색된 공고 중 사용자에게 정말로 적합한 공고가 하나도 없다고 판단되면, recommended_job_indices에 빈 배열 []을 반환하고 overall_advice에서 그 이유를 설명하세요.\n"
)
